...
def load_yaml_file(file_path):
    ...
    __LOGGER__.debug(f"Loading YAML file: {file_path}")
...
def get_git_metadata_from_github(file_path, github_token=None):
    ...
    try:
        ...
        __LOGGER__.info(f"Querying GitHub API: {url}?path={repo_path}")

        response = requests.get(url, headers=headers, params=params, timeout=10)

        if response.status_code != 200:
            __LOGGER__.warning(
                f"GitHub API returned status {response.status_code} for {repo_path}"
            )
            if response.status_code == 404:
                __LOGGER__.warning(
                    "File may not exist in the repository yet or path is incorrect"
                )
            return None
        ...
        __LOGGER__.error(f"GitHub API error for {file_path}: {e}")
        return None
...
def get_git_metadata_from_git(file_path):
    ...
    try:
        ...
        __LOGGER__.error(f"Git log error for {file_path}: {e}")
        return {"created": None, "lastUpdated": None, "contributors": []}
...
def get_git_metadata(file_path, github_token=None):
    ...
    if github_token:
        ...
        if metadata:
            return metadata
        __LOGGER__.warning(f"Falling back to git log for {file_path}")
...
def find_all_yaml_files(data_dir="data/paths"):
    ...
    if not data_path.exists():
        __LOGGER__.warning(f"Data directory '{data_dir}' does not exist")
        return []
...
def check_github_access(github_token):
    ...
    try:
        ...
        if response.status_code == 404:
            __LOGGER__.warning(f"Cannot access repository {owner}/{repo}")
            __LOGGER__.warning(
                "Repository may be private. Ensure your token has 'repo' scope"
            )
            __LOGGER__.warning("Falling back to git log for all files")
            return False
        elif response.status_code != 200:
            __LOGGER__.warning(f"GitHub API returned status {response.status_code}")
            __LOGGER__.warning("Falling back to git log for all files")
            return False

        return True
    except Exception as e:
        __LOGGER__.error(f"Error checking GitHub access: {e}")
        return False
...
def check_git_sync():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if commits_ahead > 0:
                __LOGGER__.warning(
                    f"Your branch is {commits_ahead} commit(s) ahead of origin/main"
                )
                __LOGGER__.warning("Some files may not be available via GitHub API yet")
    except Exception:
...
def convert_yaml_to_json(input_dir="data/paths", output_file="docs/paths.json"):
    ...
    __LOGGER__.info("Converting YAML files to JSON...")
    ...
    if github_token:
        ...
        __LOGGER__.warning("No GITHUB_TOKEN found, using git log")
        __LOGGER__.info("Set GITHUB_TOKEN environment variable to use GitHub API")

    yaml_files = find_all_yaml_files(input_dir)

    if not yaml_files:
        __LOGGER__.warning(f"No YAML files found in '{input_dir}'")
        return

    __LOGGER__.info(f"Found {len(yaml_files)} YAML file(s)")
    ...
    for yaml_file in yaml_files:
        try:
            __LOGGER__.info(f"Processing: {yaml_file}")
            ...
            __LOGGER__.error(error_msg)
            errors.append(error_msg)

    if errors:
        __LOGGER__.warning(f"{len(errors)} error(s) occurred during conversion")
        for error in errors:
    ...
    __LOGGER__.info(f"Successfully converted {len(paths)} path(s) to JSON")
    __LOGGER__.info(f"Output: {output_file}")

    # Generate metadata
    metadata = {
        "totalPaths": len(paths),
        "services": list(
            set([service for path in paths for service in path.get("services", [])])
        ),
        "categories": list(
            set([path.get("category") for path in paths if path.get("category")])
        ),
    ...
    __LOGGER__.info(f"Metadata: {metadata_file}")
    __LOGGER__.info("Statistics:")
    __LOGGER__.info(f"Total paths: {metadata['totalPaths']}")
    __LOGGER__.info(f"Services: {len(metadata['services'])}")
    __LOGGER__.info(f"Categories: {len(metadata['categories'])}")
